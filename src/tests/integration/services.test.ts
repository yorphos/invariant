...
vi.mock('../../lib/services/tax', () => ({
  calculateTax: vi.fn(async (subtotal: number, _taxCodeId: number, _issueDate: string, isTaxInclusive: boolean) => {
    const { getTestDatabase } = await import('./test-db');
    const db = await getTestDatabase();
    const hstRow = db.prepare("SELECT id FROM account WHERE code = '2100'").get() as { id: number } | undefined;
...
function getAccountBalance(db: Awaited<ReturnType<typeof getTestDatabase>>, accountCode: string): number {
...
describe('Integration - Service Layer Operations', () => {
  ...
  beforeEach(async () => {
    ...
    const acctByCode = new Map(
      (db.prepare('SELECT id, code FROM account').all() as Array<{ id: number; code: string }>)
        .map(a => [a.code, a.id]),
    );
  ...
  describe('Account CRUD via persistence service', () => {
    it('should create an account and return its ID', async () => {
      ...
      const row = db.prepare('SELECT * FROM account WHERE id = ?').get(id) as Record<string, unknown>;
    ...
    it('should get all accounts including seeded and created ones', async () => {
      await persistenceService.createAccount({
        code: '1500', name: 'Equipment', type: 'asset', parent_id: null, is_active: true,
      });
    ...
    it('should get an account by ID', async () => {
      const id = await persistenceService.createAccount({
        code: '1600', name: 'Land', type: 'asset', parent_id: null, is_active: true,
      });
    ...
    it('should update account name and code', async () => {
      const id = await persistenceService.createAccount({
        code: '1700', name: 'Old Name', type: 'liability', parent_id: null, is_active: true,
      });

      await persistenceService.updateAccount(id, { name: 'New Name', code: '1701' });

      const updated = db.prepare('SELECT * FROM account WHERE id = ?').get(id) as Record<string, unknown>;
    ...
    it('should deactivate an account via soft delete', async () => {
      const id = await persistenceService.createAccount({
        code: '1800', name: 'Temp Account', type: 'expense', parent_id: null, is_active: true,
      });

      await persistenceService.updateAccount(id, { is_active: false });

      const row = db.prepare('SELECT is_active FROM account WHERE id = ?').get(id) as { is_active: number };
      expect(row.is_active).toBe(0);
    });

    it('should exclude inactive accounts by default', async () => {
      await persistenceService.createAccount({
        code: '1800', name: 'Inactive Acct', type: 'expense', parent_id: null, is_active: false,
      });
  ...
  describe('Contact CRUD via persistence service', () => {
    it('should create a contact and return its ID', async () => {
      ...
      const row = db.prepare('SELECT * FROM contact WHERE id = ?').get(id) as Record<string, unknown>;
      expect(row).toBeDefined();
    ...
    it('should get all active contacts', async () => {
      await persistenceService.createContact({
        type: 'vendor', name: 'Supplier Inc', is_active: true,
      });
    ...
    it('should filter contacts by type', async () => {
      await persistenceService.createContact({
        type: 'vendor', name: 'Vendor Only', is_active: true,
      });
  ...
  describe('Journal Entry via persistence service', () => {
    it('should create a balanced journal entry', async () => {
      ...
      const arId = (db.prepare("SELECT id FROM account WHERE code = '1100'").get() as { id: number }).id;
      const revenueId = (db.prepare("SELECT id FROM account WHERE code = '4000'").get() as { id: number }).id;
      ...
      const entryId = await persistenceService.createJournalEntry(
        ...
        [
          { account_id: arId, debit_amount: 100.00, credit_amount: 0.00 },
          { account_id: revenueId, debit_amount: 0.00, credit_amount: 100.00 },
        ],
      );
    ...
    it('should persist journal lines with correct amounts', async () => {
      const eventId = await persistenceService.createTransactionEvent({
        event_type: 'manual_entry',
        description: 'Split entry', reference: 'JE-002', created_by: 'test_user',
      });

      const cashId = (db.prepare("SELECT id FROM account WHERE code = '1000'").get() as { id: number }).id;
      const officeExpId = (db.prepare("SELECT id FROM account WHERE code = '5100'").get() as { id: number }).id;
      const taxExpId = (db.prepare("SELECT id FROM account WHERE code = '5300'").get() as { id: number }).id;

      const entryId = await persistenceService.createJournalEntry(
        {
          event_id: eventId, entry_date: '2026-06-15',
          description: 'Office supplies split', reference: 'JE-002',
          status: 'posted',
        },
        [
          { account_id: officeExpId, debit_amount: 80.00, credit_amount: 0.00 },
          { account_id: taxExpId, debit_amount: 20.00, credit_amount: 0.00 },
          { account_id: cashId, debit_amount: 0.00, credit_amount: 100.00 },
        ],
      );
    ...
    it('should track event to entry relationship', async () => {
      const eventId = await persistenceService.createTransactionEvent({
        event_type: 'manual_entry', description: 'Rel test',
        reference: 'JE-003', created_by: 'test_user',
      });
      ...
      await persistenceService.createJournalEntry(
        { event_id: eventId, entry_date: '2026-07-01', description: 'Relation test', status: 'posted' },
        [
          { account_id: cashId, debit_amount: 50.00, credit_amount: 0.00 },
          { account_id: revenueId, debit_amount: 0.00, credit_amount: 50.00 },
        ],
      );
    ...
    it('should enforce journal entry balance at the database level', async () => {
      const eventId = await persistenceService.createTransactionEvent({
        event_type: 'manual_entry', description: 'Unbalanced', created_by: 'test_user',
      });
      ...
      const entryId = await persistenceService.createJournalEntry(
        ...
        [
          { account_id: cashId, debit_amount: 100.00, credit_amount: 0.00 },
        ],
      );

      await expect(
        persistenceService.postJournalEntry(entryId, 'test_user'),
      ).rejects.toThrow();
    });
  });
  ...
  describe('Invoice creation via persistence service', () => {
    it('should create an invoice and return its ID', async () => {
      const contactId = (db.prepare("SELECT id FROM contact WHERE name = 'Test Customer'").get() as { id: number }).id;

      const eventId = await persistenceService.createTransactionEvent({
        event_type: 'invoice_created', description: 'Invoice INV-001',
        reference: 'INV-001', created_by: 'test_user',
      });
      ...
      const invoiceId = await persistenceService.createInvoice(
        ...
        [
          { line_number: 1, description: 'Consulting services', quantity: 10, unit_price: 100.00, amount: 1000.00 },
        ],
      );
    ...
    it('should retrieve created invoice by ID', async () => {
      ...
      const eventId = await persistenceService.createTransactionEvent({
        event_type: 'invoice_created', description: 'Invoice INV-002',
        reference: 'INV-002', created_by: 'test_user',
      });

      const invoiceId = await persistenceService.createInvoice(
        {
          invoice_number: 'INV-002', contact_id: contactId,
          event_id: eventId, issue_date: '2026-06-15',
          due_date: '2026-07-15', status: 'sent',
          subtotal: 500.00, tax_amount: 65.00, total_amount: 565.00, paid_amount: 0.00,
        },
        [
          { line_number: 1, description: 'Product sale', quantity: 5, unit_price: 100.00, amount: 500.00 },
        ],
      );
      ...
      expect(invoice!.total_amount).toBe(565.00);
    });
    ...
    it('should retrieve invoice lines', async () => {
      ...
      const eventId = await persistenceService.createTransactionEvent({
        event_type: 'invoice_created', description: 'Invoice INV-003',
        reference: 'INV-003', created_by: 'test_user',
      });

      const invoiceId = await persistenceService.createInvoice(
        {
          invoice_number: 'INV-003', contact_id: contactId,
          event_id: eventId, issue_date: '2026-07-01',
          due_date: '2026-08-01', status: 'sent',
          subtotal: 200.00, tax_amount: 26.00, total_amount: 226.00, paid_amount: 0.00,
        },
        [
          { line_number: 1, description: 'Item A', quantity: 2, unit_price: 50.00, amount: 100.00 },
          { line_number: 2, description: 'Item B', quantity: 1, unit_price: 100.00, amount: 100.00 },
        ],
      );
    ...
    it('should reject invoice with zero quantity line item', async () => {
      ...
      await expect(
        createInvoice(
          ...
          [
            { line_number: 1, description: 'Zero qty', quantity: 0, unit_price: 100.00, amount: 0, account_id: revenueId },
          ],
          defaultContext,
        ),
    ...
    it('should reject invoice without tax code', async () => {
      ...
      await expect(
        createInvoice(
          ...
          [
            { line_number: 1, description: 'Service', quantity: 1, unit_price: 100.00, amount: 100.00, account_id: revenueId },
          ],
          defaultContext,
        ),
      ).rejects.toThrow(/tax code is required/i);
    });
  ...
  describe('Payment operations', () => {
    it('should create a payment record via persistence service', async () => {
      ...
      const eventId = await persistenceService.createTransactionEvent({
        event_type: 'payment_received', description: 'Payment PMT-001',
        reference: 'PMT-001', created_by: 'test_user',
      });
    ...
    it('should create allocations linking payment to invoices', async () => {
      ...
      const invEventId = await persistenceService.createTransactionEvent({
        event_type: 'invoice_created', description: 'Invoice INV-100',
        reference: 'INV-100', created_by: 'test_user',
      });

      const invoiceId = await persistenceService.createInvoice(
        {
          invoice_number: 'INV-100', contact_id: contactId,
          event_id: invEventId, issue_date: '2026-06-01',
          due_date: '2026-07-01', status: 'sent',
          subtotal: 1000.00, tax_amount: 130.00, total_amount: 1130.00, paid_amount: 0.00,
        },
        [
          { line_number: 1, description: 'Services', quantity: 10, unit_price: 100.00, amount: 1000.00 },
        ],
      );

      const pmtEventId = await persistenceService.createTransactionEvent({
        event_type: 'payment_received', description: 'Payment PMT-100',
        reference: 'PMT-100', created_by: 'test_user',
      });

      const paymentId = await persistenceService.createPayment({
        payment_number: 'PMT-100', contact_id: contactId,
        event_id: pmtEventId, payment_date: '2026-06-20',
        amount: 500.00, payment_method: 'check' as PaymentMethod,
        allocated_amount: 500.00, status: 'allocated',
      });
      ...
      const updatedInvoice = db.prepare('SELECT paid_amount FROM invoice WHERE id = ?').get(invoiceId) as { paid_amount: number };
      expect(updatedInvoice.paid_amount).toBe(500.00);
    });
    ...
    it('should create payment with journal entries via domain function', async () => {
      ...
      const invEventId = await persistenceService.createTransactionEvent({
        event_type: 'invoice_created', description: 'Invoice INV-200',
        reference: 'INV-200', created_by: 'test_user',
      });

      await persistenceService.createJournalEntry(
        { event_id: invEventId, entry_date: '2026-06-01', description: 'Invoice entry', status: 'posted' },
        [
          { account_id: arId, debit_amount: 1130.00, credit_amount: 0.00 },
          { account_id: revenueId, debit_amount: 0.00, credit_amount: 1000.00 },
        ],
      );

      const invoiceId = await persistenceService.createInvoice(
        {
          invoice_number: 'INV-200', contact_id: contactId,
          event_id: invEventId, issue_date: '2026-06-01',
          due_date: '2026-07-01', status: 'sent',
          subtotal: 1000.00, tax_amount: 130.00, total_amount: 1130.00, paid_amount: 0.00,
        },
      ...
      const result = await createPayment(
        ...
        [{ invoice_id: invoiceId, amount: 1130.00 }],
        defaultContext,
      );
  ...
  describe('Expense operations', () => {
    it('should create an expense journal entry via persistence service', async () => {
      const eventId = await persistenceService.createTransactionEvent({
        event_type: 'expense_recorded', description: 'Office supplies',
        reference: 'EXP-001', created_by: 'test_user',
      });

      const cashId = (db.prepare("SELECT id FROM account WHERE code = '1000'").get() as { id: number }).id;
      const expenseId = (db.prepare("SELECT id FROM account WHERE code = '5100'").get() as { id: number }).id;

      const entryId = await persistenceService.createJournalEntry(
        {
          event_id: eventId, entry_date: '2026-06-01',
          description: 'Office supplies expense', status: 'posted',
        },
        [
          { account_id: expenseId, debit_amount: 150.00, credit_amount: 0.00 },
          { account_id: cashId, debit_amount: 0.00, credit_amount: 150.00 },
        ],
      );
      ...
      expect(getAccountBalance(db, '5100')).toBe(150.00);
      expect(getAccountBalance(db, '1000')).toBe(-150.00);
    });
  ...
  describe('Error handling', () => {
    it('should reject creating an account with duplicate code', async () => {
      await persistenceService.createAccount({
        code: '9999', name: 'Unique', type: 'asset', parent_id: null, is_active: true,
      });

      await expect(
        persistenceService.createAccount({
          code: '9999', name: 'Duplicate', type: 'asset', parent_id: null, is_active: true,
        }),
      ).rejects.toThrow();
    });
    ...
    it('should reject creating an account with empty code', async () => {
      await expect(
        persistenceService.createAccount({
          code: '', name: 'No Code', type: 'asset', parent_id: null, is_active: true,
        }),
      ).rejects.toThrow();
    });
    ...
    it('should reject posting an unbalanced entry', async () => {
      const eventId = await persistenceService.createTransactionEvent({
        event_type: 'manual_entry', description: 'Unbalanced entry', created_by: 'test_user',
      });
  ...
  describe('Double-entry accounting verification', () => {
    it('should maintain balanced books after multi-step workflow', async () => {
      ...
      const event1Id = await persistenceService.createTransactionEvent({
        event_type: 'manual_entry', description: 'Revenue entry', created_by: 'test_user',
      });
      ...
      const entry1Id = await persistenceService.createJournalEntry(
        ...
        [
          { account_id: cashId, debit_amount: 1000.00, credit_amount: 0.00 },
          { account_id: revenueId, debit_amount: 0.00, credit_amount: 1000.00 },
        ],
      );
      ...
      const event2Id = await persistenceService.createTransactionEvent({
        event_type: 'manual_entry', description: 'Expense entry', created_by: 'test_user',
      });
      ...
      const entry2Id = await persistenceService.createJournalEntry(
        ...
        [
          { account_id: expenseId, debit_amount: 300.00, credit_amount: 0.00 },
          { account_id: cashId, debit_amount: 0.00, credit_amount: 300.00 },
        ],
      );
      expect(isJournalEntryBalanced(db, entry2Id)).toBe(true);

      expect(getAccountBalance(db, '1000')).toBeCloseTo(700.00, 2);
      expect(getAccountBalance(db, '4000')).toBeCloseTo(1000.00, 2);
      expect(getAccountBalance(db, '5100')).toBeCloseTo(300.00, 2);

...
describe('Integration - Credit Notes & Refunds', () => {
  ...
  async function createContact(
    name: string,
    type: 'customer' | 'vendor' | 'both' = 'customer',
  ): Promise<number> {
    const result = db
      .prepare(`
  ...
  async function createInvoice(
    ...
    const eventId = db
      ...
      VALUES (?, ?, ?, ?)
    `)
      .run('invoice_created', `Invoice ${invoiceNumber}`, invoiceNumber, 'test_user')
    ...
    const entryId = db
      ...
      VALUES (?, ?, ?, ?, ?)
    `)
      .run(eventId, '2026-01-25', 'Invoice Entry', invoiceNumber, 'draft')
      .lastInsertRowid as number;
  ...
  describe('Credit Note CRUD Operations', () => {
    it('should create a credit note', async () => {
      ...
      const creditNote = db
        .prepare('SELECT * FROM credit_note WHERE id = ?')
      ...
      expect(creditNote.subtotal).toBe(1000.0);
      expect(creditNote.tax_amount).toBe(130.0);
      expect(creditNote.total_amount).toBe(1130.0);
      expect(creditNote.applied_amount).toBe(0.0);
    });
    ...
    it('should read a credit note', async () => {
      ...
      const creditNoteId = db
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .run('CN-002', contactId, '2026-01-25', 'issued', 500.0, 65.0, 565.0, 0.0, 'Test read')
        .lastInsertRowid as number;
    ...
    it('should enforce unique credit note number constraint', async () => {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run('CN-004', contactId, '2026-01-25', 'draft', 1000.0, 130.0, 1130.0, 0.0);

      expect(() => {
        db.prepare(`
          INSERT INTO credit_note (credit_note_number, contact_id, issue_date, status, subtotal, tax_amount, total_amount, applied_amount)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run('CN-004', contactId, '2026-01-25', 'draft', 500.0, 65.0, 565.0, 0.0);
      }).toThrow();
    });
    ...
    it('should enforce status check constraint', async () => {
      ...
      expect(() => {
        db.prepare(`
          INSERT INTO credit_note (credit_note_number, contact_id, issue_date, status, subtotal, tax_amount, total_amount, applied_amount)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run('CN-005', contactId, '2026-01-25', 'invalid_status', 1000.0, 130.0, 1130.0, 0.0);
      }).toThrow();
    });
  });
  ...
  describe('Credit Note Line CRUD Operations', () => {
    it('should create credit note lines', async () => {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(creditNoteId, 1, 'Product A', 10, 100.0, 1000.0, 0);

      db.prepare(`
        INSERT INTO credit_note_line (credit_note_id, line_number, description, quantity, unit_price, amount, is_tax_inclusive)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(creditNoteId, 2, 'Product B', 5, 50.0, 250.0, 0);

      const lines = db
        .prepare('SELECT * FROM credit_note_line WHERE credit_note_id = ? ORDER BY line_number')
        .all(creditNoteId) as any[];
    ...
    it('should read credit note lines', async () => {
      ...
      db.prepare(`
        INSERT INTO credit_note_line (credit_note_id, line_number, description, quantity, unit_price, amount, is_tax_inclusive)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(creditNoteId, 1, 'Test Item', 1, 100.0, 100.0, 0);

      const line = db
        .prepare('SELECT * FROM credit_note_line WHERE credit_note_id = ?')
        .get(creditNoteId) as any;
  ...
  describe('Credit Note Application CRUD Operations', () => {
    it('should create credit note application', async () => {
      ...
      const invoiceId = await createInvoice('INV-001', contactId, 1130.0);

      db.prepare(`
        INSERT INTO credit_note_application (credit_note_id, invoice_id, amount, application_date, notes)
        VALUES (?, ?, ?, ?, ?)
      `).run(creditNoteId, invoiceId, 500.0, '2026-01-26', 'Partial application');

      const application = db
        .prepare('SELECT * FROM credit_note_application WHERE credit_note_id = ?')
        .get(creditNoteId) as any;

      expect(application).toBeDefined();
      expect(application.invoice_id).toBe(invoiceId);
      expect(application.amount).toBe(500.0);
    });
    ...
    it('should read credit note applications by credit note', async () => {
      ...
      const invoiceId1 = await createInvoice('INV-001', contactId, 1130.0);
      const invoiceId2 = await createInvoice('INV-002', contactId, 565.0);

      db.prepare(`
        INSERT INTO credit_note_application (credit_note_id, invoice_id, amount, application_date)
        VALUES (?, ?, ?, ?)
      `).run(creditNoteId, invoiceId1, 500.0, '2026-01-26');

      db.prepare(`
        INSERT INTO credit_note_application (credit_note_id, invoice_id, amount, application_date)
        VALUES (?, ?, ?, ?)
      `).run(creditNoteId, invoiceId2, 265.0, '2026-01-27');
    ...
    it('should read credit note applications by invoice', async () => {
      ...
      const invoiceId = await createInvoice('INV-001', contactId, 2260.0);

      db.prepare(`
        INSERT INTO credit_note_application (credit_note_id, invoice_id, amount, application_date)
        VALUES (?, ?, ?, ?)
      `).run(creditNoteId1, invoiceId, 500.0, '2026-01-26');

      db.prepare(`
        INSERT INTO credit_note_application (credit_note_id, invoice_id, amount, application_date)
        VALUES (?, ?, ?, ?)
      `).run(creditNoteId2, invoiceId, 300.0, '2026-01-27');
    ...
    it('should update applied amount on application', async () => {
      ...
      db.prepare(`
        INSERT INTO credit_note_application (credit_note_id, invoice_id, amount, application_date)
        VALUES (?, ?, ?, ?)
      `).run(creditNoteId, invoiceId, 500.0, '2026-01-26');

      creditNote = db.prepare('SELECT * FROM credit_note WHERE id = ?').get(creditNoteId) as any;
      expect(creditNote.applied_amount).toBe(500.0);
    });
    ...
    it('should cascade delete applications when credit note is deleted', async () => {
      ...
      const applicationsBefore = db
        .prepare('SELECT COUNT(*) as count FROM credit_note_application WHERE credit_note_id = ?')
        .get(creditNoteId) as any;
      expect(applicationsBefore.count).toBe(1);

      db.prepare('DELETE FROM credit_note WHERE id = ?').run(creditNoteId);

      const applicationsAfter = db
        .prepare('SELECT COUNT(*) as count FROM credit_note_application WHERE credit_note_id = ?')
        .get(creditNoteId) as any;
  ...
  describe('Credit Note Refund CRUD Operations', () => {
    it('should create a refund', async () => {
      ...
      const refundId = db
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `)
        .run(creditNoteId, 'REF-001', '2026-01-26', 500.0, 'cash', 'Cash refund')
        .lastInsertRowid as number;

      const refund = db
        .prepare('SELECT * FROM credit_note_refund WHERE id = ?')
        .get(refundId) as any;

      expect(refund).toBeDefined();
      expect(refund.credit_note_id).toBe(creditNoteId);
      expect(refund.refund_number).toBe('REF-001');
      expect(refund.amount).toBe(500.0);
    ...
    it('should read refunds by credit note', async () => {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `).run(creditNoteId, 'REF-001', '2026-01-26', 500.0, 'cash');

      db.prepare(`
        INSERT INTO credit_note_refund (credit_note_id, refund_number, refund_date, amount, payment_method)
        VALUES (?, ?, ?, ?, ?)
      `).run(creditNoteId, 'REF-002', '2026-01-27', 630.0, 'transfer');

      const refunds = db
        .prepare('SELECT * FROM credit_note_refund WHERE credit_note_id = ?')
        .all(creditNoteId) as any[];
    ...
    it('should enforce unique refund number constraint', async () => {
      ...
      db.prepare(`
        INSERT INTO credit_note_refund (credit_note_id, refund_number, refund_date, amount, payment_method)
        VALUES (?, ?, ?, ?, ?)
      `).run(creditNoteId, 'REF-003', '2026-01-26', 500.0, 'cash');

      expect(() => {
        db.prepare(`
          INSERT INTO credit_note_refund (credit_note_id, refund_number, refund_date, amount, payment_method)
          VALUES (?, ?, ?, ?, ?)
        `).run(creditNoteId, 'REF-003', '2026-01-27', 630.0, 'transfer');
      }).toThrow();
    });
    ...
    it('should enforce payment method check constraint', async () => {
      ...
      expect(() => {
        db.prepare(`
          INSERT INTO credit_note_refund (credit_note_id, refund_number, refund_date, amount, payment_method)
          VALUES (?, ?, ?, ?, ?)
        `).run(creditNoteId, 'REF-004', '2026-01-26', 500.0, 'invalid_method');
      }).toThrow();
    });
    ...
    it('should cascade delete refunds when credit note is deleted', async () => {
      ...
      db.prepare(`
        INSERT INTO credit_note_refund (credit_note_id, refund_number, refund_date, amount, payment_method)
        VALUES (?, ?, ?, ?, ?)
      `).run(creditNoteId, 'REF-005', '2026-01-26', 500.0, 'cash');

      const refundsBefore = db
        .prepare('SELECT COUNT(*) as count FROM credit_note_refund WHERE credit_note_id = ?')
        .get(creditNoteId) as any;
      expect(refundsBefore.count).toBe(1);

      db.prepare('DELETE FROM credit_note WHERE id = ?').run(creditNoteId);

      const refundsAfter = db
        .prepare('SELECT COUNT(*) as count FROM credit_note_refund WHERE credit_note_id = ?')
        .get(creditNoteId) as any;
  ...
  describe('Database Triggers', () => {
    it('should update subtotal when credit note line is inserted', async () => {
      ...
      const creditNoteId = db
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
        .run('CN-020', contactId, '2026-01-25', 'draft', 0.0, 0.0, 0.0, 0.0, 'Test trigger')
        .lastInsertRowid as number;

      let creditNote = db
        .prepare('SELECT * FROM credit_note WHERE id = ?')
        .get(creditNoteId) as any;
      expect(creditNote.subtotal).toBe(0.0);
    ...
    it('should update subtotal when multiple credit note lines are inserted', async () => {
      ...
      db.prepare(`
        INSERT INTO credit_note_line (credit_note_id, line_number, description, quantity, unit_price, amount, is_tax_inclusive)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(creditNoteId, 3, 'Product C', 2, 75.0, 150.0, 0);

      const creditNote = db
        .prepare('SELECT * FROM credit_note WHERE id = ?')
        .get(creditNoteId) as any;
      expect(creditNote.subtotal).toBe(1400.0);
    });
    ...
    it('should update subtotal when credit note line is deleted', async () => {
      ...
      let creditNote = db
        .prepare('SELECT * FROM credit_note WHERE id = ?')
        .get(creditNoteId) as any;
      expect(creditNote.subtotal).toBe(1250.0);

      db.prepare('DELETE FROM credit_note_line WHERE credit_note_id = ? AND line_number = 2').run(
        creditNoteId,
      );
    ...
    it('should update applied amount when credit note application is inserted', async () => {
      ...
      creditNote = db.prepare('SELECT * FROM credit_note WHERE id = ?').get(creditNoteId) as any;
      expect(creditNote.applied_amount).toBe(765.0);
    });
    ...
    it('should update applied amount when credit note application is deleted', async () => {
      ...
      db.prepare(
        'DELETE FROM credit_note_application WHERE credit_note_id = ? AND invoice_id = ?',
      ).run(creditNoteId, invoiceId1);

      creditNote = db.prepare('SELECT * FROM credit_note WHERE id = ?').get(creditNoteId) as any;
      expect(creditNote.applied_amount).toBe(265.0);
    });
  });
    ...
    it('should enforce foreign key constraint on credit_note.contact_id', async () => {
      expect(() => {
        db.prepare(`
          INSERT INTO credit_note (credit_note_number, contact_id, issue_date, status, subtotal, tax_amount, total_amount, applied_amount)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run('CN-025', 9999, '2026-01-25', 'draft', 1000.0, 130.0, 1130.0, 0.0);
      }).toThrow();
    });
    ...
    it('should enforce foreign key constraint on credit_note_line.credit_note_id', async () => {
      expect(() => {
        db.prepare(`
          INSERT INTO credit_note_line (credit_note_id, line_number, description, quantity, unit_price, amount, is_tax_inclusive)
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(9999, 1, 'Test Item', 1, 100.0, 100.0, 0);
      }).toThrow();
    });
    ...
    it('should enforce foreign key constraint on credit_note_application.credit_note_id', async () => {
      ...
      expect(() => {
        db.prepare(`
          INSERT INTO credit_note_application (credit_note_id, invoice_id, amount, application_date)
          VALUES (?, ?, ?, ?)
        `).run(9999, invoiceId, 500.0, '2026-01-26');
      }).toThrow();
    });
    ...
    it('should enforce foreign key constraint on credit_note_application.invoice_id', async () => {
      ...
      expect(() => {
        db.prepare(`
          INSERT INTO credit_note_application (credit_note_id, invoice_id, amount, application_date)
          VALUES (?, ?, ?, ?)
        `).run(creditNoteId, 9999, 500.0, '2026-01-26');
      }).toThrow();
    });
    ...
    it('should enforce foreign key constraint on credit_note_refund.credit_note_id', async () => {
      expect(() => {
        db.prepare(`
          INSERT INTO credit_note_refund (credit_note_id, refund_number, refund_date, amount, payment_method)
          VALUES (?, ?, ?, ?, ?)
        `).run(9999, 'REF-001', '2026-01-26', 500.0, 'cash');
      }).toThrow();
    });
  });
  ...
  describe('Full Workflow Integration', () => {
    ...
    it('should handle partial and multiple applications', async () => {
      ...
      const invoiceId1 = await createInvoice('INV-001', contactId, 1130.0);
      const invoiceId2 = await createInvoice('INV-002', contactId, 565.0);
      const invoiceId3 = await createInvoice('INV-003', contactId, 226.0);
      ...
      db.prepare(`
        INSERT INTO credit_note_application (credit_note_id, invoice_id, amount, application_date)
        VALUES (?, ?, ?, ?)
      `).run(creditNoteId, invoiceId3, 226.0, '2026-01-28');

      const creditNote = db
        .prepare('SELECT * FROM credit_note WHERE id = ?')
        .get(creditNoteId) as any;
      expect(creditNote.applied_amount).toBe(991.0);
    ...
    it('should handle credit note with refunds', async () => {
      ...
      const refunds = db
        ...
        .all(creditNoteId) as any[];
      expect(refunds.length).toBe(2);
      expect(refunds[0].amount).toBe(500.0);
      expect(refunds[1].amount).toBe(630.0);
    });
    ...
    it('should handle credit note with both applications and refunds', async () => {
      ...
      db.prepare(`
        INSERT INTO credit_note_refund (credit_note_id, refund_number, refund_date, amount, payment_method)
        VALUES (?, ?, ?, ?, ?)
      `).run(creditNoteId, 'REF-001', '2026-01-27', 630.0, 'cash');
  ...
  describe('Index Verification', () => {
    it('should use index on credit_note_number for lookups', async () => {
      ...
      const query = db
        .prepare('EXPLAIN QUERY PLAN SELECT * FROM credit_note WHERE credit_note_number = ?')
        .all('CN-031');
      const usesIndex = query.some(
        (row: any) => row.detail && row.detail.includes('credit_note_number'),
      );
    ...
    it('should use index on credit_note_contact for contact lookups', async () => {
      ...
      const query = db
        .prepare('EXPLAIN QUERY PLAN SELECT * FROM credit_note WHERE contact_id = ?')
        .all(contactId);
    ...
    it('should use index on credit_note_status for status lookups', async () => {
      ...
      const query = db
        .prepare('EXPLAIN QUERY PLAN SELECT * FROM credit_note WHERE status = ?')
        .all('issued');

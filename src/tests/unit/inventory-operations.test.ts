...
import type {
  ...
  SaleInput,
  AdjustmentInput,
} from '../../lib/domain/inventory-operations';
...
describe('Inventory Adjustment - Validation', () => {
  ...
  it('should require unit cost for positive adjustments', () => {
    ...
    const isValidWithCost =
      adjustmentWithCost.quantity > 0 &&
      adjustmentWithCost.unit_cost !== undefined &&
      adjustmentWithCost.unit_cost >= 0;
    const isValidWithoutCost =
      adjustmentWithoutCost.quantity > 0 && adjustmentWithoutCost.unit_cost !== undefined;
...
describe('FIFO Cost Calculation', () => {
  ...
  function calculateFIFOCOGS(
    layers: FIFOLayer[],
    quantityToSell: number,
  ): { cogs: number; remainingLayers: FIFOLayer[] } {
  ...
  it('should calculate COGS using FIFO for single purchase layer', () => {
    const layers: FIFOLayer[] = [{ purchase_date: '2026-01-01', quantity: 100, unit_cost: 50.0 }];

    const result = calculateFIFOCOGS(layers, 20);

    expect(result.cogs).toBe(1000.0); // 20 × $50
  ...
  it('should calculate COGS using FIFO for multiple purchase layers', () => {
    const layers: FIFOLayer[] = [
      { purchase_date: '2026-01-01', quantity: 50, unit_cost: 40.0 },
      { purchase_date: '2026-01-10', quantity: 50, unit_cost: 45.0 },
      { purchase_date: '2026-01-20', quantity: 50, unit_cost: 50.0 },
    ];
    ...
    expect(result.cogs).toBe(3125.0);
    expect(result.remainingLayers.length).toBe(2);
    expect(result.remainingLayers[0].quantity).toBe(25); // Remaining from second layer
    expect(result.remainingLayers[0].unit_cost).toBe(45.0);
    expect(result.remainingLayers[1].quantity).toBe(50); // Third layer untouched
    expect(result.remainingLayers[1].unit_cost).toBe(50.0);
  });

  it('should handle exact layer consumption', () => {
    const layers: FIFOLayer[] = [
      { purchase_date: '2026-01-01', quantity: 100, unit_cost: 50.0 },
      { purchase_date: '2026-01-10', quantity: 50, unit_cost: 55.0 },
    ];

    const result = calculateFIFOCOGS(layers, 100);

    expect(result.cogs).toBe(5000.0); // 100 × $50
    expect(result.remainingLayers.length).toBe(1);
    expect(result.remainingLayers[0].quantity).toBe(50);
    expect(result.remainingLayers[0].unit_cost).toBe(55.0);
  });

  it('should handle sale quantity exceeding available inventory', () => {
    const layers: FIFOLayer[] = [{ purchase_date: '2026-01-01', quantity: 50, unit_cost: 50.0 }];

    const result = calculateFIFOCOGS(layers, 100);

    // Should only consume what's available
    expect(result.cogs).toBe(2500.0); // 50 × $50
    expect(result.remainingLayers.length).toBe(0);
  });

  it('should calculate correct COGS with varying unit costs', () => {
    const layers: FIFOLayer[] = [
      { purchase_date: '2026-01-01', quantity: 10, unit_cost: 10.0 },
      { purchase_date: '2026-01-05', quantity: 10, unit_cost: 15.0 },
      { purchase_date: '2026-01-10', quantity: 10, unit_cost: 20.0 },
    ];
    ...
    expect(result.cogs).toBe(350.0);
    expect(result.remainingLayers.length).toBe(1);
    expect(result.remainingLayers[0].quantity).toBe(5);
    expect(result.remainingLayers[0].unit_cost).toBe(20.0);
  });

  it('should handle fractional quantities in FIFO calculation', () => {
    const layers: FIFOLayer[] = [
      { purchase_date: '2026-01-01', quantity: 10.5, unit_cost: 50.0 },
      { purchase_date: '2026-01-10', quantity: 15.3, unit_cost: 55.0 },
    ];
    ...
    expect(result.cogs).toBeCloseTo(651.5, 2);
    expect(result.remainingLayers.length).toBe(1);
...
describe('Inventory Valuation', () => {
  it('should calculate average cost correctly', () => {
    const purchases = [
      { quantity: 100, unit_cost: 40.0 },
      { quantity: 50, unit_cost: 50.0 },
      { quantity: 150, unit_cost: 45.0 },
    ];
  ...
  it('should calculate inventory value at average cost', () => {
    ...
    expect(inventoryValue).toBeCloseTo(6625.0, 1); // Use 1 decimal place tolerance
  });
});
...
describe('Inventory Edge Cases', () => {
  ...
  it('should handle very large quantities', () => {
    ...
    const totalCost = purchase.quantity * purchase.unit_cost;
    expect(totalCost).toBe(500000.0);
  });
  ...
  it('should handle items with no purchase history (negative inventory scenario)', () => {
    ...
    const quantitySold = Math.min(
      saleQty,
      layers.reduce((sum, l) => sum + l.quantity, 0),
    );
    const cogs = 0;
...
describe('Inventory Accounting Principles', () => {
  ...
  it('should ensure COGS is recorded at historical cost (FIFO)', () => {
    ...
    const historicalCost = 80.0; // FIFO cost from purchase layers

    const cogs = historicalCost; // Not salePrice!
    const grossProfit = salePrice - cogs;

    expect(cogs).toBe(80.0);
    expect(grossProfit).toBe(70.0);
  });

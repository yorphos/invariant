...
async function getCachedAccounts(): Promise<Map<number, Account>> {
  ...
  if (!accountsCache || now - accountsCacheTimestamp > CACHE_TTL_MS) {
    ...
    accountsCache = await db.select<Account[]>('SELECT * FROM account ORDER BY code');
...
export class PersistenceService {
  ...
  async getAccounts(includeInactive: boolean = false): Promise<Account[]> {
    const db = await getDatabase();
    if (includeInactive) {
      return await db.select<Account[]>('SELECT * FROM account ORDER BY code');
    }
    return await db.select<Account[]>('SELECT * FROM account WHERE is_active = 1 ORDER BY code');
  }
  ...
  async createTransactionEvent(
    event: Omit<TransactionEvent, 'id' | 'created_at'>,
  ): Promise<number> {
    const db = await getDatabase();
    const result = await db.execute(
  ...
  async getJournalEntry(id: number): Promise<JournalEntry | null> {
    const db = await getDatabase();
    const results = await db.select<JournalEntry[]>('SELECT * FROM journal_entry WHERE id = ?', [
      id,
    ]);
  ...
  async getJournalLines(journalEntryId: number): Promise<JournalLine[]> {
    const db = await getDatabase();
    return await db.select<JournalLine[]>('SELECT * FROM journal_line WHERE journal_entry_id = ?', [
      journalEntryId,
    ]);
  }
   ...
   */
  async getJournalEntriesWithLines(
    limit: number = 50,
  ...
  async getContacts(type?: 'customer' | 'vendor' | 'both'): Promise<Contact[]> {
    ...
    return await db.select<Contact[]>('SELECT * FROM contact WHERE is_active = 1 ORDER BY name');
  }
  ...
  async updateContact(
    ...
    if (contact.type !== undefined) {
      const existingContact = await db.select<Contact[]>('SELECT * FROM contact WHERE id = ?', [
        id,
      ]);
      ...
      if (newType !== 'both' && currentType !== newType) {
        ...
        if ((currentType === 'customer' || currentType === 'both') && newType === 'vendor') {
          ...
          if (invoiceCount[0].count > 0) {
            throw new Error(
              'Cannot change to vendor-only: contact has existing invoices. Change to "Both" instead.',
            );
          }
          ...
          if (paymentCount[0].count > 0) {
            throw new Error(
              'Cannot change to vendor-only: contact has existing payments. Change to "Both" instead.',
            );
          }
        }
   ...
   */
  async getAvailableContactTypes(
    contactId: number,
  ): Promise<Array<'customer' | 'vendor' | 'both'>> {
    const db = await getDatabase();
  ...
  async getInvoices(contactId?: number): Promise<Invoice[]> {
    ...
    return await db.select<Invoice[]>('SELECT * FROM invoice ORDER BY issue_date DESC');
  }
  ...
  async getInvoiceById(id: number): Promise<Invoice | null> {
    const db = await getDatabase();
    const results = await db.select<Invoice[]>('SELECT * FROM invoice WHERE id = ?', [id]);
    return results.length > 0 ? results[0] : null;
  }
  ...
  async getPayments(contactId?: number): Promise<Payment[]> {
    ...
    return await db.select<Payment[]>('SELECT * FROM payment ORDER BY payment_date DESC');
  }

  async getPaymentById(id: number): Promise<Payment | null> {
    const db = await getDatabase();
    const results = await db.select<Payment[]>('SELECT * FROM payment WHERE id = ?', [id]);
    return results.length > 0 ? results[0] : null;
  }

  async updatePayment(
    id: number,
    payment: Partial<Omit<Payment, 'id' | 'created_at' | 'updated_at'>>,
  ): Promise<void> {
    const db = await getDatabase();
    const fields: string[] = [];
    const values: SqlParams = [];
  ...
  async getAllocations(paymentId?: number, invoiceId?: number): Promise<Allocation[]> {
    const db = await getDatabase();
    if (paymentId) {
      return await db.select<Allocation[]>('SELECT * FROM allocation WHERE payment_id = ?', [
        paymentId,
      ]);
    }
    if (invoiceId) {
      return await db.select<Allocation[]>('SELECT * FROM allocation WHERE invoice_id = ?', [
        invoiceId,
      ]);
    }
  ...
  async getBills(vendorId?: number): Promise<Bill[]> {
    ...
    return await db.select<Bill[]>('SELECT * FROM bill ORDER BY bill_date DESC');
  }
  ...
  async getBillById(id: number): Promise<Bill | null> {
    const db = await getDatabase();
    const results = await db.select<Bill[]>('SELECT * FROM bill WHERE id = ?', [id]);
    return results.length > 0 ? results[0] : null;
  }
  ...
  async createVendorPayment(
    payment: Omit<VendorPayment, 'id' | 'created_at' | 'updated_at'>,
  ): Promise<number> {
    const db = await getDatabase();
    const result = await db.execute(
  ...
  async getVendorPaymentById(id: number): Promise<VendorPayment | null> {
    const db = await getDatabase();
    const results = await db.select<VendorPayment[]>('SELECT * FROM vendor_payment WHERE id = ?', [
      id,
    ]);
    return results.length > 0 ? results[0] : null;
  }
  ...
  async createBillAllocation(
    allocation: Omit<BillAllocation, 'id' | 'created_at'>,
  ): Promise<number> {
    const db = await getDatabase();
    const result = await db.execute(
  ...
  async getBillAllocations(vendorPaymentId?: number, billId?: number): Promise<BillAllocation[]> {
    ...
    if (billId) {
      return await db.select<BillAllocation[]>('SELECT * FROM bill_allocation WHERE bill_id = ?', [
        billId,
      ]);
    }
  ...
  async getCreditNotes(contactId?: number): Promise<CreditNote[]> {
    ...
    return await db.select<CreditNote[]>('SELECT * FROM credit_note ORDER BY issue_date DESC');
  }

  async getCreditNoteById(id: number): Promise<CreditNote | null> {
    const db = await getDatabase();
    const results = await db.select<CreditNote[]>('SELECT * FROM credit_note WHERE id = ?', [id]);
    return results.length > 0 ? results[0] : null;
  }
  ...
  async getCreditNoteApplications(
    creditNoteId?: number,
    invoiceId?: number,
    ...
    return await db.select<CreditNoteApplication[]>(
      'SELECT * FROM credit_note_application ORDER BY application_date DESC',
    );
  }
}

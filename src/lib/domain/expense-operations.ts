...
export async function createExpense(expenseData: ExpenseInput, context: PolicyContext) {
  try {
    ...
    if (expenseAccount.type !== 'expense') {
      throw new Error(
        `Account "${expenseAccount.name}" must be an expense account. Selected account is of type "${expenseAccount.type}".`,
      );
    }
    if (paymentAccount.type !== 'asset') {
      throw new Error(
        `Payment account "${paymentAccount.name}" must be an asset account (cash/bank). Selected account is of type "${paymentAccount.type}".`,
      );
    }

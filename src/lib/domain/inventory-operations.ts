...
export async function createItem(itemData: ItemInput): Promise<{ item_id: number }> {
  ...
  const existing = await db.select<Item[]>('SELECT id FROM item WHERE sku = ? LIMIT 1', [
    itemData.sku,
  ]);
...
export async function recordPurchase(
  ...
  // Get item details
  const items = await db.select<Item[]>('SELECT * FROM item WHERE id = ? LIMIT 1', [
  ...
  await db.execute(
    `INSERT INTO journal_line (journal_entry_id, account_id, debit_amount, credit_amount, description)
     VALUES (?, ?, ?, 0.00, ?)`,
    [journalEntryId, item.inventory_account_id, totalCost, `Inventory - ${item.name}`],
  );

  await db.execute(
    `INSERT INTO journal_line (journal_entry_id, account_id, debit_amount, credit_amount, description)
     VALUES (?, ?, 0.00, ?, ?)`,
    [journalEntryId, purchaseData.cash_account_id, totalCost, `Payment for inventory`],
  );
...
export async function recordSale(
  ...
  const eventResult = await db.execute(
    `INSERT INTO transaction_event (event_type, description, reference, created_at)
     VALUES (?, ?, ?, datetime('now'))`,
    ['inventory_sale', `Sale ${saleData.quantity} × ${item.name}`, saleData.reference || null],
  );
  ...
  await db.execute(
    `INSERT INTO journal_line (journal_entry_id, account_id, debit_amount, credit_amount, description)
     VALUES (?, ?, ?, 0.00, ?)`,
    [journalEntryId, item.cogs_account_id, cogsCalc.cogs_amount, `COGS - ${item.name}`],
  );
 ...
 */
export async function getInventoryValuation(asOfDate: string): Promise<
  Array<{
    item: Item;
...
> {
  ...
  const items = await db.select<Item[]>(`SELECT * FROM item WHERE is_active = 1 ORDER BY sku`);

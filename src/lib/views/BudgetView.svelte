<script lang="ts">
  ...
  async function loadBudgets() {
    try {
      budgets = await getBudgets(
        filterFiscalYear !== '' ? Number(filterFiscalYear) : undefined
      );
    } catch (e) {
  ...
  function getLineAmount(accountId: number, period: number): number {
    const line = formLines.find(
      (l) => l.account_id === accountId && l.period === period
    );
    return line ? line.amount : 0;
  }

  function setLineAmount(accountId: number, period: number, amount: number) {
    const existing = formLines.findIndex(
      (l) => l.account_id === accountId && l.period === period
    );
    if (existing >= 0) {
      ...
      formLines = [
        ...formLines,
        { account_id: accountId, period, amount, notes: '' },
      ];
    }
  }
  ...
  async function handleUpdate() {
    ...
    try {
      await updateBudgetLines(editingBudget.id, formLines.map((l) => ({
        account_id: l.account_id,
        period: l.period,
        amount: l.amount,
        notes: l.notes || undefined,
